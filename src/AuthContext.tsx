...
const OFFLINE_SESSION_KEY = "radar_offline_session_v1";
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const ensureAdvisor = (account: any) => {
    if (account?.rol !== "ASESOR" || !account?.id_asesor) {
      throw new Error("Esta aplicación es exclusiva para asesores vinculados. Usa el sistema web para funciones administrativas.");
    }
  };
  ...
  const persistValidatedSession = async (accessToken: string, account: User) => {
    ...
    await Promise.all([
      ...
      secureStore.set(ADVISOR_KEY, String(account.id_asesor)),
  ...
  const clearSession = async () => {
    ...
    await Promise.all([
      secureStore.remove(TOKEN_KEY), secureStore.remove(ADVISOR_KEY), secureStore.remove(OFFLINE_SESSION_KEY),
    ]);
  };

  const complete = async (data: any) => {
    ensureAdvisor(data.user);
