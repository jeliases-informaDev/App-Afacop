...
const advisorDisplayName = (user: {
  ...
  return [names, firstSurname].filter(Boolean).join(" ") || user?.username || "Asesor";
};
...
export default function MoreScreen() {
  ...
  const openSettings = async () => {
    ...
    catch { Alert.alert("Ajustes no disponibles", "Abre los ajustes del teléfono y selecciona Mi Radar 360°."); }
  };
  const showFieldHelp = () => Alert.alert("Gestión en campo", "Mantén activa la ubicación durante tu jornada. Antes de registrar una visita, verifica la fotografía, firma y descripción de la gestión.");
  ...
  return <Screen>
    ...
    <View style={s.profile}>
      ...
      <View style={s.roleBadge}><View style={s.roleDot} /><Text style={s.role}>{user?.rol || "ASESOR"}</Text></View>
    </View>
    <Card>
