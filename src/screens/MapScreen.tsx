...
function normalizePoint(item: any) {
  ...
  return {
    ...client,
    id: client.id_cliente || item.id_cliente || item.id,
    latitud: Number(client.latitud),
    longitud: Number(client.longitud),
    estado:
      item.estado_visita === "VISITADO"
        ? "GESTIONADO"
        : item.estado_visita || client.estado || "PENDIENTE",
  };
}
  ...
  const center = async () => {
    try {
      ...
      if (permission.status !== "granted")
        throw new Error("Permite la ubicación para centrar el mapa.");
        ...
        >
          {filters.map((item) => (
            <Pressable
              ...
              style={[s.chip, filter === item ? s.chipOn : null]}
            >
              <View
                style={[
                  s.chipDot,
      ...
      {!loading && points.length === 0 ? (
        <View style={s.empty}>
          <MaterialCommunityIcons
            ...
            color={C.primary}
          />
          <Text style={s.emptyTitle}>Sin clientes ubicados</Text>
          <Text style={s.emptyText}>
            {error || "No hay coordenadas disponibles para este filtro."}
          </Text>
        </View>
      ) : null}
    </View>
