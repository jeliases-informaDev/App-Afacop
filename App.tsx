...
function Shell() {
  ...
  const { location, error: locationError } = useAdvisorLocation(api, Boolean(token));
  ...
  if (loading)
    return (
      <View style={s.loading}>
        <MaterialCommunityIcons name="radar" size={42} color={C.primary} />
        <ActivityIndicator size="large" color={C.primary} />
        <Text style={s.loadingText}>Preparando Mi Radar 360°</Text>
      </View>
    );
      ...
      {!online && !offlineSession ? (
        <View style={[s.offline, { paddingTop: Math.max(insets.top, 8) }]}>
          ...
          <Text style={s.offlineText}>
            Sin conexión · tus gestiones se guardarán y sincronizarán después
          </Text>
        </View>
      ) : null}
